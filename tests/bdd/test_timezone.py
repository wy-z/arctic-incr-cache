...
def _lib_with_tz_bars(ctx, n, tz_name, start):
    ...
    raw.index = pd.DatetimeIndex(raw.index).tz_convert(tz)
...
@then(parsers.parse('the result has timezone "{tz_name}"'))
def _result_has_tz(ctx, tz_name):
    idx = pd.DatetimeIndex(ctx["result"].index)
    assert idx.tz is not None
    assert str(idx.tz) == tz_name
...
def _result_in_ny_time(ctx):
    ...
    assert ctx["result"].index[0] < pd.Timestamp("2024-01-15 12:00:00", tz=tz)
